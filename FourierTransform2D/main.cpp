...
// do not perform the inverse DFT
	...
	if (idft == false)
	{
		...
		if (rank == 0)
		{
			...
			image.SaveImageData(finalOutput_dft.c_str(), data1, width, height);	//save dft
		}
	}
...
// perform the inverse DFT after writing the dft
	...
	else
	{
		...
		Transpose(data2, data1, width, height);							//transpose matrix
		if (DEBUG) debug("006_after_transpose_after2D_rank", data1, data2, width, height);
		if (rank == 0) image.SaveImageData(finalOutput_dft.c_str(), data1, width, height);		//save dft transform
...
//		Transform_test(data2, width, data1);
		...
		if (rank == 0)
		{
			...
			image.SaveImageData(finalOutput_idft.c_str(), data1, width, height);	//save dft
		}
	}
...
void Transform1D(Complex* h, int w, Complex* H, bool idft)
{
	...
	for(int i = startRow; i <= endRow; i++)
	{
		...
		for(int n = 0; n < w; n++)
		{
			...
			if (idft == true) H[i*w+n] = H[i*w+n]/w;
		}
